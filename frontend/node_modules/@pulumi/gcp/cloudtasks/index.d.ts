export { GetQueueIamPolicyArgs, GetQueueIamPolicyResult, GetQueueIamPolicyOutputArgs } from "./getQueueIamPolicy";
export declare const getQueueIamPolicy: typeof import("./getQueueIamPolicy").getQueueIamPolicy;
export declare const getQueueIamPolicyOutput: typeof import("./getQueueIamPolicy").getQueueIamPolicyOutput;
export { QueueArgs, QueueState } from "./queue";
export type Queue = import("./queue").Queue;
export declare const Queue: typeof import("./queue").Queue;
export { QueueIamBindingArgs, QueueIamBindingState } from "./queueIamBinding";
export type QueueIamBinding = import("./queueIamBinding").QueueIamBinding;
export declare const QueueIamBinding: typeof import("./queueIamBinding").QueueIamBinding;
export { QueueIamMemberArgs, QueueIamMemberState } from "./queueIamMember";
export type QueueIamMember = import("./queueIamMember").QueueIamMember;
export declare const QueueIamMember: typeof import("./queueIamMember").QueueIamMember;
export { QueueIamPolicyArgs, QueueIamPolicyState } from "./queueIamPolicy";
export type QueueIamPolicy = import("./queueIamPolicy").QueueIamPolicy;
export declare const QueueIamPolicy: typeof import("./queueIamPolicy").QueueIamPolicy;
//# sourceMappingURL=index.d.ts.map